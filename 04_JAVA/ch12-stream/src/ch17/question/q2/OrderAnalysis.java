...
public class OrderAnalysis {
    public static void main(String[] args) {
        ...
        // 결과: [1, 4]
        List<Integer> IdsOver100 = orders.stream()
                .filter(order -> order.getTotalAmount() >= 1000000)
                .map(Order::getOrderId).toList();
        System.out.println("결과 : " + IdsOver100);
        ...
        List<String> productNames = orders.stream()
                .filter(order -> order.getOrderDate().isAfter(LocalDate.of(2023, 5, 1)))
                .flatMap(order -> order.getProducts().stream())
                .map(Product::getName)
                .distinct()
                .toList();

        System.out.println("결과 : " + productNames);
        ...
        System.out.println("결과 : " + avgPriceByCategory);
        ...
        System.out.println("결과 : " + totalAmountByDate);
    }
}